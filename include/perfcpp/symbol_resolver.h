...
public:
  class Module {
  public:
    Module(std::string&& name, const std::uintptr_t start, const std::uintptr_t end, const std::size_t offset, std::string&& path, std::string&& permission) noexcept
      : _name(std::move(name)), _start(start), _end(end), _offset(offset), _path(std::move(path)), _permissions(std::move(permission)) {
    }
    ...
    [[nodiscard]] bool operator==(const Module &other) const
    {
  ...
  {
  public:
    std::size_t operator()(const Module& module) const
    {
      return std::hash<std::string>()(module.path());
    }
  };

  class Symbol {
  public:
    Symbol(std::string&& name, const std::uintptr_t address, const std::size_t size) noexcept : _name(std::move(name)), _address(address), _size(size) { }
    ~Symbol() = default;
    ...
    [[nodiscard]] std::string to_string() const { auto name = _name; return name.append(" (").append(std::to_string(_address)).append("--").append(std::to_string(_address + size())).append(")"); }
  private:
    std::string _name;
  ...
  public:
    ResolvedSymbol(const Module& module, const Symbol& symbol, const std::size_t offset) noexcept : _module(module), _symbol(symbol), _offset(offset) { }
    ...
    [[nodiscard]] std::string to_string() const { auto str = std::string{"["}; return str.append(_module.name()).append("] ").append(_symbol.name()).append("+").append(std::to_string(_offset)); }
  private:
...
private:
   ...
   */
  [[nodiscard]] static std::optional<ResolvedSymbol> resolve(const Module& module, const std::vector<Symbol>& symbols, std::uintptr_t logical_instruction_pointer) noexcept;
