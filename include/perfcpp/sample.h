...
public:
   ...
   * Set the raw data.
   *
   * @param raw_data Raw data to set.
   */
  void raw_data(std::vector<std::byte>&& raw_data) noexcept { _raw_data.emplace(std::move(raw_data)); }
   ...
   */
  [[nodiscard]] const std::optional<std::vector<std::byte>>& raw_data() const noexcept { return _raw_data; }
...
private:
  ...
  std::optional<std::vector<std::byte>> _raw_data{ std::nullopt };
