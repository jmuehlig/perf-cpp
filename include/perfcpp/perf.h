...
private:
    ...
    public:
        explicit Event(std::string&& name) noexcept
            : _name(std::move(name)), _is_hidden(false), _is_counter(false), _group_id(0U), _in_group_id(0U)
        {
        }

        Event(std::string&& name, const bool is_hidden, const std::uint8_t group_id, const std::uint8_t in_group_id) noexcept :
            _name(std::move(name)), _is_hidden(is_hidden), _is_counter(true), _group_id(group_id), _in_group_id(in_group_id)
        {
        }
...
{
public:
    PerfMT(Perf&& perf, std::uint16_t num_threads);

    PerfMT(const Perf& perf, const std::uint16_t num_threads) : PerfMT(perf::Perf{perf}, num_threads) { }
...
private:
    std::vector<perf::Perf> _thread_local_counter;
};
}