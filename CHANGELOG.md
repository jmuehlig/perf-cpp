...
- `cmake` no longer builds `Release` with debug information; use `-DCMAKE_BUILD_TYPE=RelWithDebInfo` for an optimized build with full debug information. This shrinks the installed static library from roughly 65 MB to 2.5 MB.
- `clang-tidy` is now controlled by the `ENABLE_CLANG_TIDY` option (default `ON` when building *perf-cpp* standalone, `OFF` otherwise), so embedded and packaged builds no longer lint the library.
- `cmake` no longer forces `CMAKE_CXX_STANDARD` to `17` when the consuming toolchain already requested a newer standard.
