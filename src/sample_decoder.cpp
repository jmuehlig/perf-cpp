...
perf::SampleDecoder::decode_sample_id_all(SampleIterator& entry, Sample& sample) const noexcept
{
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_TIME)) {
    sample.metadata().timestamp(entry.read<std::uint64_t>());
  }

  if (this->_sampler_values.is_set(PERF_SAMPLE_STREAM_ID)) {
    sample.metadata().stream_id(entry.read<std::uint64_t>());
  }

  if (this->_sampler_values.is_set(PERF_SAMPLE_CPU)) {
...

{
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_ADDR)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_PERIOD)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_WEIGHT)) {
    ...
    if (HardwareInfo::is_intel()) {
      ...
      sample.data_access().latency().cache_miss(weight);
                                        ...
                                        /// Linux 5.12
  if (this->_sampler_values.is_set(PERF_SAMPLE_WEIGHT_STRUCT)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_PHYS_ADDR)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_CGROUP)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_DATA_PAGE_SIZE)) {
  ...
  if (this->_sampler_values.is_set(PERF_SAMPLE_CODE_PAGE_SIZE)) {
...
{
  if (const auto& raw = sample.raw(); raw.has_value()) {
    const auto ibs_fetch_decoder = IBSFetchDecoder{ raw.value() };
...
{
  if (const auto& raw = sample.raw(); raw.has_value()) {
    const auto ibs_op_decoder = IBSOpDecoder{ raw.value() };
