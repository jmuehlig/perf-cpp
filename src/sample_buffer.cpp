...
perf::SampleBuffer::SampleBuffer(const UniqueFileDescriptor& file_descriptor, std::uint64_t count_buffer_pages)
{
  ...
  this->_mmap_buffer = MmapBuffer{ file_descriptor, true, count_buffer_pages };

  /// Create an event file descriptor to cancel the thread when closing the sample buffer.
  this->_cancel_thread_event_file_descriptor = UniqueFileDescriptor{ ::eventfd(0, 0) };
  ...
  this->_poll_and_handle_ringbuffer_overflow_thread =
    std::thread(&SampleBuffer::poll_and_handle_ringbuffer_overflow,
                this,
                FileDescriptorView{ file_descriptor },
                FileDescriptorView{ this->_cancel_thread_event_file_descriptor });
}
...
perf::SampleBuffer::consume_sample_data()
{
  ...
  auto sample_buffers = std::exchange(this->_sample_buffers, std::vector<std::vector<std::byte>>{});
  this->_sample_buffers.reserve(sample_buffers.size());
...
void
perf::SampleBuffer::poll_and_handle_ringbuffer_overflow(const FileDescriptorView perf_file_descriptor,
                                                        const FileDescriptorView cancel_file_descriptor)
{
  do {
    /// Initialize the file descriptor set.
    auto file_descriptor_set = ::fd_set{};
    FD_ZERO(&file_descriptor_set);
    FD_SET(perf_file_descriptor.value(), &file_descriptor_set);
    FD_SET(cancel_file_descriptor.value(), &file_descriptor_set);

    const auto max_file_descriptor = std::max(perf_file_descriptor.value(), cancel_file_descriptor.value()) + 1;
    ...
    if (select > 0) {
      ...
      /// buffer.
      if (FD_ISSET(perf_file_descriptor.value(), &file_descriptor_set)) {
