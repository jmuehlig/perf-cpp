...
int main()
{
    std::cout << "libperf-cpp example: Record perf samples including time, instruction pointer, and cpu id for single-threaded random access to an in-memory array." << std::endl;
    ...
    auto sampler = perf::Sampler{
        counter_definitions,
        "cycles", /// Event that generates an overflow which is samples (here we sample every 1,000,000th cycle)
        perf::Sampler::Type::Time | perf::Sampler::Type::Period | perf::Sampler::Type::InstructionPointer | perf::Sampler::Type::CPU, /// Controls what to include into the sample, see https://man7.org/linux/man-pages/man2/perf_event_open.2.html
        perf_config
    };
    ...
    for (auto index = 0U; index < count_show_samples; ++index)
    {
        ...
        if (sample.time().has_value() && sample.period().has_value() && sample.instruction_pointer().has_value() && sample.cpu_id().has_value())
        {
            std::cout << "Time = " << sample.time().value() << " | Period = " << sample.period().value() << " | Instruction Pointer = 0x" << std::hex << sample.instruction_pointer().value() << std::dec << " | CPU ID = " << sample.cpu_id().value() << "\n";
        }
    }
    std::cout << std::flush;
